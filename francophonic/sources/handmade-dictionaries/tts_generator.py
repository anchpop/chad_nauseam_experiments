...
def main():
    ...
    sentences = set([item for sublist in [[translation['sentence'] for translation in translations['frenchTranslations']] for translations in sentences_dictionary] for item in sublist])
    ...
    previous_files = set([f.split(".")[0] for f in listdir(dir) if isfile(join(dir, f))])
    ...
    inp = input(f"To synthesize speech for these {len(candidates_to_tts)} sentences would cost around ${characters / 1000000 * 20}, continue? (yes/no) ")
    if inp == "yes": 
